...
var asyncNum1 = setting.asyncNum1;	//获取主页链接并发连接数
var asyncNum2 = setting.asyncNum2;	//获取Email并发连接数
...
function start(){
	...
	eventproxy.all("getFollowersLinks", "getFollowingLinks", function(followersLinks, followingLinks){
		...
		console.log("全部跟随者及关注者的主页链接已抓取完毕，即将异步并发抓取所有跟随者及关注者的Email，当前并发数为：" + asyncNum2);
		//根据所有user的主页链接并发抓取相对应的Email
		async.mapLimit(allFollowLinks, asyncNum2, function(Link, callback){
			getHomepage(Link).then((result) => {
				count++;
...
function getLinks(type, all, links){
	getNum(type).then((result) => {
		...
		console.log("即将异步并发抓取所有 " + type + " 主页链接，当前并发数为：" + asyncNum1);
		async.mapLimit(queryList, asyncNum1, function(query, callback){
			console.log("正在抓取 page = "+ query + " 的 " + type + " 列表");
			getPage(type, query).then((result) => { 
				var $ = cheerio.load(result.text);
				//当前所在页所有的跟随者或关注者
